...
class DectylConsole {
                ...
                if (first[i] == "%") {
                    ...
                    if (a < args.length) {
                        ...
                        if (__char == "s") {
                            ...
                            formattedArg = this.#inspect(args[a++], rInspectOptions);
                        } else if (__char == "c") {
                            a++;
                            formattedArg = "";
                        }
    ...
    constructor(print){
        this.#print = print;
        this.#inspect = Deno?.inspect.bind(Deno) ?? function inspect(value) {
            return String(value);
        };
    }
    log(...args) {
        this.#print(`${this.#inspectArgs(args, {
            ...INSPECT_OPTIONS,
...
const inspect1 = globalThis.Deno.inspect.bind(Deno);
...
class DeployWorkerHost {
    ...
    async #handleMessage(evt) {
            ...
            case "fetch":
                {
                    ...
                    this.#context.dispatchEvent(new FetchEvent(new Request(input, requestInit), (response)=>this.#postResponse(id, response)
                    ));
                    break;
                }
            default:
     ...
     #log(level, ...data) {
        this.#postMessage({
            type: "internalLog",
            level,
            messages: data.map((v)=>typeof v === "string" ? v : inspect1(v)
            )
        });
    }
     #parseInit(id, init) {
        ...
        switch(init.body.type){
            ...
            case "urlsearchparams":
                ...
                break;
            case "stream":
                assert2(!this.#bodyControllers.has(id));
        ...
        if (init.signal) {
            assert2(!this.#signalControllers.has(id));
    ...
    async #postResponse(id, response) {
    ...
    constructor(){
        ...
        Object.defineProperties(globalThis, {
            "addEventListener": createValueDesc(context.addEventListener.bind(context)),
            "console": createNonEnumDesc(console),
            "dispatchEvent": createValueDesc(context.dispatchEvent.bind(context)),
            "removeEventListener": createValueDesc(context.removeEventListener.bind(context))
        });
    }
}
